...
public partial class AddMaterialModal : Window
{
    ...
    private void AddButton_Click(object sender, RoutedEventArgs e)
    {
        ...
        {
            MessageBox.Show("Имя материала не может быть пустым.", "Ошибка", MessageBoxButton.OK,
                MessageBoxImage.Error);
            return;
        }

        // Проверка, что цена материала введена корректно
        if (!int.TryParse(MaterialPriceTextBox.Text, out var price) || price <= 0)
        {
            MessageBox.Show("Введите корректное положительное числовое значение для цены материала.", "Ошибка",
